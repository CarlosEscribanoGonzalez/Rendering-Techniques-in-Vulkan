...
namespace MiniEngine
{
    ...
    namespace UtilsVK
    {
        ...
        void createImage(const DeviceVK& i_device, VkFormat i_format, VkImageUsageFlagBits i_usage_bits, uint32_t i_width, uint32_t i_height, ImageBlock& o_image_block, int sampleCount = 1);
        ...
        int getMultisampleCount_AA();
        VkSampleCountFlagBits getSampleCountFlag_AA();
        VkSampleCountFlagBits getCountFlag(int sampleCount);
        ...
        void createTLAS(const DeviceVK& i_device, std::vector<Matrix4f>& i_transforms, std::vector<VkAccelerationStructureKHR>& i_blas_instances,
            ...
            VkBuffer& o_instances_buffer, VkDeviceMemory& o_instances_memory);
        void updateTLAS(const DeviceVK& i_device, std::vector<Matrix4f>& i_transforms, std::vector<VkAccelerationStructureKHR>& i_blas_instances,
            VkBuffer& o_buffer, VkDeviceMemory& o_memory, VkAccelerationStructureKHR& o_tlas);
        uint64_t get_device_address(VkDevice i_device, VkBuffer i_buffer);

        void copyAttachment(const DeviceVK& i_device, const VkCommandBuffer& current_cmd, ImageBlock& attachment_1, ImageBlock& attachment_2,
