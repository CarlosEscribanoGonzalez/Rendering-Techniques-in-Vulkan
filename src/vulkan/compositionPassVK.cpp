...
CompositionPassVK::CompositionPassVK(
    ...
    const ImageBlock& i_input_attachment,
    std::array<ImageBlock, 3> i_output_swap_images
) :
    RenderPassVK(i_runtime),
    m_input_attachment(i_input_attachment),
...
void CompositionPassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        image_infos[0].sampler = m_input_attachment.m_sampler;
        image_infos[0].imageView = m_input_attachment.m_image_view;
