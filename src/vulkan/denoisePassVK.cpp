...
DenoisePassVK::DenoisePassVK(
    ...
    m_color_attachment(i_in_attachment),
    m_depth_attachment(i_in_depth_attachment),
    m_normal_attachment(i_in_normal_attachment),
    m_history_attachment(i_in_history_attachment),
    m_motion_attachment(i_in_motion_attachment),
    m_blur_attachment(i_blur_attachment)
{
    ...
    switch (type) {
    case BlurType::Gauss3x3:
        createMask(3);
        break;
    case BlurType::Gauss5x5:
        createMask(5);
        break;
    case BlurType::Gauss7x7:
        createMask(7);
        break;
    case BlurType::Gauss9x9:
        createMask(9);
        break;
    case BlurType::Gauss13x13:
        createMask(13);
        break;
    }
}
...
void DenoisePassVK::createMask(int width, float sigma)
{
    if (sigma <= 0.0f)
        sigma = width / 6.0f;
    int half = width / 2;
    ...
    for (int y = -half; y <= half; y++) {
        for (int x = -half; x <= half; x++) {
            ...
            float value = exp(-dist2 / (2.0f * sigma * sigma));
            kernel[(x + half) + (y + half) * width] = value;
            sum += value;
        }
    }
    for (float& v : kernel)
    ...
    for (int i = 0; i < size; i++)
    {
        int vec_index = i / 4;
        int comp_index = i % 4;
        m_mask.mask[vec_index][comp_index] = kernel[i];
    }
    m_mask.width = width;
    ...
    void* kernel_data;
    vkMapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_mask_memory, 0,
        sizeof(MaskData), 0, &kernel_data);
    memcpy(kernel_data, &m_mask, sizeof(MaskData));
    vkUnmapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_mask_memory);
}

bool DenoisePassVK::initialize()
{
    ...
    {
        {
            VkShaderModule vert_module = m_runtime.m_shader_registry->loadShader("./shaders/fullScreen_v.spv", VK_SHADER_STAGE_VERTEX_BIT);
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader("./shaders/denoising_f.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void DenoisePassVK::shutdown()
{
    ...
    vkDestroyPipeline(renderer.getDevice()->getLogicalDevice(), m_ssao_pipeline, nullptr);
    ...
    vkDestroyBuffer(renderer.getDevice()->getLogicalDevice(), m_mask_buffer, nullptr);
    vkFreeMemory(renderer.getDevice()->getLogicalDevice(), m_mask_memory, nullptr);
}


VkCommandBuffer DenoisePassVK::draw(const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    clear_values.color = { { 1.0f, 1.0f, 1.0f, 1.0f } };
    ...
    UtilsVK::copyAttachment(*m_runtime.m_renderer->getDevice(), current_cmd, m_blur_attachment,
        m_history_attachment, width, height);
...
void DenoisePassVK::createFbo()
{
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_blur_attachment.m_image_view;
...
void DenoisePassVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;

    std::array<VkAttachmentDescription, 1> attachments = {};
    // SSAO attachment
    attachments[0].format = m_blur_attachment.m_format;
    ...
    subpass_description.pColorAttachments = &ssao_reference;
...
void DenoisePassVK::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    if (vkCreateGraphicsPipelines(renderer.getDevice()->getLogicalDevice(), VK_NULL_HANDLE, 1, graphic_pipelines.data(), nullptr, &m_ssao_pipeline))
    {
...
void DenoisePassVK::createDescriptorLayout()
{
    ...
    set_info.bindingCount = layout_bindings.size();
    set_info.pBindings = layout_bindings.data();

    if (VK_SUCCESS != vkCreateDescriptorSetLayout(m_runtime.m_renderer->getDevice()->getLogicalDevice(), &set_info, nullptr, &m_descriptor_set_layout))
        throw MiniEngineException("Error creating descriptor set");
}


void DenoisePassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        std::array<VkDescriptorImageInfo, 5> images_info;
        images_info[0].sampler = m_color_attachment.m_sampler;
        images_info[0].imageView = m_color_attachment.m_image_view;
        images_info[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        images_info[1].sampler = m_depth_attachment.m_sampler;
        images_info[1].imageView = m_depth_attachment.m_image_view;
        images_info[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        images_info[2].sampler = m_normal_attachment.m_sampler;
        images_info[2].imageView = m_normal_attachment.m_image_view;
        images_info[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        images_info[3].sampler = m_history_attachment.m_sampler;
        images_info[3].imageView = m_history_attachment.m_image_view;
        images_info[3].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        images_info[4].sampler = m_motion_attachment.m_sampler;
        images_info[4].imageView = m_motion_attachment.m_image_view;
        images_info[4].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        VkDescriptorBufferInfo maskInfo;
        maskInfo.buffer = m_mask_buffer;
        maskInfo.offset = 0;
        maskInfo.range = sizeof(MaskData);

        std::vector<VkWriteDescriptorSet> set_write;
        VkWriteDescriptorSet set = {};
        set = {};
        set.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        set.pNext = nullptr;
        set.dstBinding = 0;
        set.dstSet = m_descriptor_sets[i].m_textures_descriptor;
        set.descriptorCount = 1;
        set.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        set.pImageInfo = nullptr;
        set.pBufferInfo = &binfo;
        set_write.push_back(set);
