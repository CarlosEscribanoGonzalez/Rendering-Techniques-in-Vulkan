...
PPPassVK::PPPassVK(
    ...
    const ImageBlock& i_output_attachment,
    PostProcessType type
) :
    RenderPassVK(i_runtime),
    m_input_attachments(i_input_attachments),
    m_output_attachment(i_output_attachment),
    type(type)
{
...
void PPPassVK::getShaderNames(char** out_names) {
    out_names[0] = "./shaders/fullScreen_v.spv";
    switch (type) {
        case(PostProcessType::FXAA):
            out_names[1] = "./shaders/fxaa_f.spv";
            break;
        case(PostProcessType::ChromaticAberration):
            out_names[1] = "./shaders/chromaticAberration_f.spv";
            break;
        case(PostProcessType::ToneMapping):
            out_names[1] = "./shaders/toneMapping_f.spv";
            break;
        case(PostProcessType::Bloom):
            out_names[1] = "./shaders/bloom_f.spv";
            break;
        case(PostProcessType::ColorAdd):
            out_names[1] = "./shaders/colorAddition_f.spv";
            break;
    }
}

bool PPPassVK::initialize()
{
    ...
    {
        {
            ...
            VkShaderModule vert_module = m_runtime.m_shader_registry->loadShader(shaderNames[0], VK_SHADER_STAGE_VERTEX_BIT);
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader(shaderNames[1], VK_SHADER_STAGE_FRAGMENT_BIT);
...
void PPPassVK::shutdown()
{
    ...
    vkDestroyPipeline(renderer.getDevice()->getLogicalDevice(), m_pp_pipeline, nullptr);
...
VkCommandBuffer PPPassVK::draw(const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    vkCmdBindPipeline(current_cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pp_pipeline);
...
void PPPassVK::createFbo()
{
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_output_attachment.m_image_view;
...
void PPPassVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;

    std::array<VkAttachmentDescription, 1> attachments = {};
    // Output attachment
    attachments[0].format = m_output_attachment.m_format;
...
void PPPassVK::createPipelines()
{
    ...
    if (vkCreateGraphicsPipelines(renderer.getDevice()->getLogicalDevice(), VK_NULL_HANDLE, 1, graphic_pipelines.data(), nullptr, &m_pp_pipeline))
    {
...
void PPPassVK::createDescriptorLayout()
{
    std::vector<VkDescriptorSetLayoutBinding> bindings (m_input_attachments.size() + 1);
...
void PPPassVK::createDescriptors()
{
    ...
    for (uint32_t j = 0; j < m_runtime.m_renderer->getWindow().getImageCount(); j++)
    {
        ...
        vkAllocateDescriptorSets(m_runtime.m_renderer->getDevice()->getLogicalDevice(), &alloc_info, &m_descriptor_sets[j].m_textures_descriptor);

        VkDescriptorBufferInfo binfo;
        binfo.buffer = m_runtime.getPerFrameBuffer()[j];
        binfo.offset = 0;
        binfo.range = sizeof(PerFrameData);

        std::vector<VkDescriptorImageInfo> image_infos (m_input_attachments.size());
        for (int i = 0; i < image_infos.size(); i++) {
            image_infos[i].sampler = m_input_attachments[i]->m_sampler;
            image_infos[i].imageView = m_input_attachments[i]->m_image_view;
            image_infos[i].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        }
        
        std::vector<VkWriteDescriptorSet> set_write (m_input_attachments.size() + 1);
        set_write[0] = {};
        set_write[0].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
        set_write[0].pNext = nullptr;
        set_write[0].dstBinding = 0;
        set_write[0].dstSet = m_descriptor_sets[j].m_textures_descriptor;
        ...
        for (int i = 1; i < set_write.size(); i++) {
            set_write[i] = {};
            set_write[i].sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
            set_write[i].pNext = nullptr;
            set_write[i].dstBinding = i;
            set_write[i].dstSet = m_descriptor_sets[j].m_textures_descriptor;
            set_write[i].descriptorCount = 1;
            set_write[i].descriptorType = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
            set_write[i].pImageInfo = &image_infos[i - 1];
        }
