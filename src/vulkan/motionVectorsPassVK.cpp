...
MotionVectorsPassVK::MotionVectorsPassVK(                             
    ...
    m_motion_output               ( i_motion_output                )
{
...
bool MotionVectorsPassVK::initialize()
{
    ...
    //SHADER STAGES
    {
        { // difuse
            ...
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader("./shaders/motionVectors_f.spv", VK_SHADER_STAGE_FRAGMENT_BIT );
...
void MotionVectorsPassVK::shutdown()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
VkCommandBuffer MotionVectorsPassVK::draw( const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void MotionVectorsPassVK::createFbo()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    for( size_t i = 0; i < m_fbos.size(); i++ )
    {
        ...
        attachments[ 0 ] = m_motion_output.m_image_view;
...
void MotionVectorsPassVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;

    std::array<VkAttachmentDescription, 1> attachments = {};
    // Color attachment
    attachments[ 0 ].format         = m_motion_output.m_format;
...
void MotionVectorsPassVK::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void MotionVectorsPassVK::createDescriptorLayout()
{
...
void MotionVectorsPassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        image_infos[1].sampler = m_in_material_attachment.m_sampler;
        image_infos[1].imageView = m_in_material_attachment.m_image_view;
        ...
        set_write[3].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        set_write[3].pBufferInfo = &binfo[1];
        set_write[3].pImageInfo = nullptr;
        set_write[3].pTexelBufferView = nullptr;
