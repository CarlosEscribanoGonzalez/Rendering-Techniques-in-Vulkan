...
LightingPassVK::LightingPassVK(                             
    ...
    const ImageBlock& i_in_blur_attachment,
    const ImageBlock& i_in_shadows_attachment,
    const ImageBlock& i_lighting_output
                          ) :
    ...
    m_in_blur_attachment          ( i_in_blur_attachment           ),
    m_in_shadow_attachment        ( i_in_shadows_attachment        ),
    m_lighting_output             ( i_lighting_output              )
{
...
bool LightingPassVK::initialize()
{
    ...
    {
        { // difuse
            std::string frag_name = runtimeVariables.shadowsType == ShadowsType::ShadowMap ? 
                "./shaders/lighting_f.spv" : "./shaders/lighting_rtx.spv";
            VkShaderModule vert_module = m_runtime.m_shader_registry->loadShader( "./shaders/fullScreen_v.spv", VK_SHADER_STAGE_VERTEX_BIT   );
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader( frag_name, VK_SHADER_STAGE_FRAGMENT_BIT );
...
VkCommandBuffer LightingPassVK::draw( const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void LightingPassVK::createFbo()
{
    ...
    for( size_t i = 0; i < m_fbos.size(); i++ )
    {
        ...
        attachments[ 0 ] = m_lighting_output.m_image_view;
...
void LightingPassVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;

    std::array<VkAttachmentDescription, 1> attachments = {};
    // Color attachment
    attachments[ 0 ].format         = m_lighting_output.m_format;
...
void LightingPassVK::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void LightingPassVK::createDescriptorLayout()
{
    ...
    layout_bindings[ 8 ].descriptorType               = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
...
void LightingPassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        binfo[0].buffer = m_runtime.getPerFrameBuffer()[i];
        binfo[0].offset = 0;
        binfo[0].range = sizeof(PerFrameData);

        binfo[1].buffer = m_runtime.getPerObjectBuffer()[i];
        binfo[1].offset = 0;
        ...
        image_infos[4].sampler = m_in_blur_attachment.m_sampler;
        image_infos[4].imageView = m_in_blur_attachment.m_image_view;
        ...
        set_write[8].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
        set_write[8].pBufferInfo = &binfo[1];
        set_write[8].pImageInfo = nullptr;
        set_write[8].pTexelBufferView = nullptr;
