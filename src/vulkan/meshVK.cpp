...
bool MeshVK::initialize()
{
    ...
    //BLAS creation:
    UtilsVK::createBLAS(*m_runtime.m_renderer->getDevice(), m_data_buffer, m_indices_buffer, m_vertices, m_indices,
        m_blas, m_blas_buffer, m_blas_memory);
    UtilsVK::setObjectName(m_runtime.m_renderer->getDevice()->getLogicalDevice(), (uint64_t)m_blas_buffer, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, "BLAS BUFFER");
    UtilsVK::setObjectTag(m_runtime.m_renderer->getDevice()->getLogicalDevice(), (uint64_t)m_blas_memory, VK_DEBUG_REPORT_OBJECT_TYPE_BUFFER_EXT, 0, m_path.size(), m_path.c_str());
...
VkBuffer MeshVK::createVertexBuffer( const std::vector<Vertex>& i_data, VkDeviceMemory& i_memory )
{
    ...
    VkMemoryPropertyFlags flags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT
        | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR;
    UtilsVK::createBuffer( *m_runtime.m_renderer->getDevice(), size, flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertex_buffer, i_memory );
...
void MeshVK::createIndexBuffer()
{
    ...
    VkMemoryPropertyFlags flags = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT
        | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT | VK_BUFFER_USAGE_ACCELERATION_STRUCTURE_BUILD_INPUT_READ_ONLY_BIT_KHR;
    UtilsVK::createBuffer( *m_runtime.m_renderer->getDevice(), size, flags, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_indices_buffer, m_indices_memory );
