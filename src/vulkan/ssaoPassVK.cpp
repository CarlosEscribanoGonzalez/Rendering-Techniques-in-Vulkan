...
bool SSAOPassVK::initialize()
{
    ...
    for (unsigned int i = 0; i < kSSAO_KERNEL_SIZE; ++i)
    {
        Vector4f sample(
        ...
        float scale = (float)i / kSSAO_KERNEL_SIZE;
    ...
    UtilsVK::createBuffer(
        *m_runtime.m_renderer->getDevice(),
        sizeof(Vector4f) * kSSAO_KERNEL_SIZE,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        m_kernel_buffer,
        m_kernel_buffer_memory
    );
    void* kernel_data;
    vkMapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_kernel_buffer_memory, 0, 
        sizeof(Vector4f) * kSSAO_KERNEL_SIZE, 0, &kernel_data);
    memcpy(kernel_data, ssaoKernel.data(), sizeof(Vector4f) * kSSAO_KERNEL_SIZE);
    vkUnmapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_kernel_buffer_memory);

    //Noise:
    std::vector<Vector4f> ssaoNoise;
    for (unsigned int i = 0; i < kSSAO_NOISE_SIZE; i++) 
    {
        Vector4f noise(
            randomFloats(generator) * 2.0f - 1.0f,
            randomFloats(generator) * 2.0f - 1.0f,
    ...
    UtilsVK::createBuffer(
        *m_runtime.m_renderer->getDevice(),
        sizeof(Vector4f) * kSSAO_NOISE_SIZE,
        VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        m_noise_buffer,
        m_noise_buffer_memory
    );
    void* noise_data;
    vkMapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_noise_buffer_memory, 0, 
        sizeof(Vector4f) * kSSAO_NOISE_SIZE, 0, &noise_data);
    memcpy(noise_data, ssaoNoise.data(), sizeof(Vector4f) * kSSAO_NOISE_SIZE);
    vkUnmapMemory(m_runtime.m_renderer->getDevice()->getLogicalDevice(), m_noise_buffer_memory);
...
void SSAOPassVK::shutdown()
{
    ...
    vkDestroyBuffer(renderer.getDevice()->getLogicalDevice(), m_kernel_buffer, nullptr);
    vkFreeMemory(renderer.getDevice()->getLogicalDevice(), m_kernel_buffer_memory, nullptr);
    vkDestroyBuffer(renderer.getDevice()->getLogicalDevice(), m_noise_buffer, nullptr);
    vkFreeMemory(renderer.getDevice()->getLogicalDevice(), m_noise_buffer_memory, nullptr);
}


VkCommandBuffer SSAOPassVK::draw(const Frame& i_frame)
{
    if (!runtimeVariables.ao_enabled) return VK_NULL_HANDLE;
    RendererVK& renderer = *m_runtime.m_renderer;
...
void SSAOPassVK::createFbo()
{
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_ssao_attachment.m_image_view;
...
void SSAOPassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        image_infos[2].sampler = m_in_color_attachment.m_sampler;
        image_infos[2].imageView = m_in_color_attachment.m_image_view;
        image_infos[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
        //Kernel:
        VkDescriptorBufferInfo kernelInfo;
        kernelInfo.buffer = m_kernel_buffer;
        kernelInfo.offset = 0;
        kernelInfo.range = sizeof(Vector4f) * kSSAO_KERNEL_SIZE;
        //Noise:
        VkDescriptorBufferInfo noiseInfo;
        noiseInfo.buffer = m_noise_buffer;
        noiseInfo.offset = 0;
        noiseInfo.range = sizeof(Vector4f) * kSSAO_NOISE_SIZE;
        ...
        set_write[4].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        set_write[4].pBufferInfo = &kernelInfo;
        //Noise:
        set_write[5] = {};
        ...
        set_write[5].descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
        set_write[5].pBufferInfo = &noiseInfo;
