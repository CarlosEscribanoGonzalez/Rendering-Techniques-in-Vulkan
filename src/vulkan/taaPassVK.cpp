...
TAAPassVK::TAAPassVK(
    ...
    m_history_attachment(i_history_attachment),
    m_motion_attachment(i_motion_attachment),
    m_output_attachment(i_aa_output)
{
    ...
    {
        {
            ...
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader("./shaders/taa_f.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void TAAPassVK::shutdown()
{
    ...
    vkDestroyPipeline(renderer.getDevice()->getLogicalDevice(), m_taa_pipeline, nullptr);
...
VkCommandBuffer TAAPassVK::draw(const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    vkCmdBindPipeline(current_cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, m_taa_pipeline);
    ...
    UtilsVK::copyAttachment(*m_runtime.m_renderer->getDevice(), current_cmd, m_output_attachment, m_history_attachment, width, height);
...
void TAAPassVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void TAAPassVK::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    if (vkCreateGraphicsPipelines(renderer.getDevice()->getLogicalDevice(), VK_NULL_HANDLE, 1, graphic_pipelines.data(), nullptr, &m_taa_pipeline))
    {
...
void TAAPassVK::createDescriptorLayout()
{
...
void TAAPassVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        image_infos[0].sampler = m_color_attachment.m_sampler;
        image_infos[0].imageView = m_color_attachment.m_image_view;
        image_infos[0].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        image_infos[1].sampler = m_history_attachment.m_sampler;
        image_infos[1].imageView = m_history_attachment.m_image_view;
        image_infos[1].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        image_infos[2].sampler = m_motion_attachment.m_sampler;
        image_infos[2].imageView = m_motion_attachment.m_image_view;
        image_infos[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        std::array<VkWriteDescriptorSet, 4> set_write;
        set_write[0] = {};
