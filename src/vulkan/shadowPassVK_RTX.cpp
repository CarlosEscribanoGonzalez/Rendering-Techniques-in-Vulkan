...
bool ShadowPassVK_RTX::initialize()
{
    ...
    {
        {
            ...
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader("./shaders/shadows_f.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
...
void ShadowPassVK_RTX::shutdown()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
VkCommandBuffer ShadowPassVK_RTX::draw(const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void ShadowPassVK_RTX::createFbo()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_shadow_buffer.m_image_view;
...
void ShadowPassVK_RTX::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;

    std::array<VkAttachmentDescription, 1> attachments = {};
    // Color attachment
    attachments[0].format = m_shadow_buffer.m_format;
...
void ShadowPassVK_RTX::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void ShadowPassVK_RTX::createDescriptorLayout()
{
    ...
    layout_bindings[3].descriptorType = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
...
void ShadowPassVK_RTX::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        VkDescriptorBufferInfo binfo[1];
        ...
        set_write[3].pNext = &writeDescriptorSetAccelerationStructure;
        set_write[3].dstBinding = 3;
        set_write[3].dstSet = m_descriptor_sets[i].m_textures_descriptor;
        set_write[3].descriptorCount = 1;
        set_write[3].descriptorType = VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR;
