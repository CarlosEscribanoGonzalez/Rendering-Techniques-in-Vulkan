...
Vector4f Camera::getCascadeSplits() const 
{
    int numCascades = runtimeVariables.num_cascades;
    if (numCascades > kMAX_CASCADES) {
        std::cout << "Hay más cascadas que el máximo posible. Reduciendo el valor a " << kMAX_CASCADES << std::endl;
        numCascades = kMAX_CASCADES;
    }
    Vector4f splits;
    for (int i = 1; i <= numCascades - 1; i++) {
        float split_log = m_near * pow(m_far / m_near, (float)i / numCascades);
        float split_linear = m_near + (m_far - m_near) * ((float)i / numCascades);
        float split = kLAMBDA * split_log + (1 - kLAMBDA) * split_linear;
        splits[i - 1] = split;
    }
    splits[numCascades - 1] = m_far;
    return splits;
}
